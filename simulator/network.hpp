...
public:
     ...
     */
	struct Data {
		Logic_Value state = Logic_Value::HI_Z;  //! The network's state, defaults to HI_Z
		std::set<Input*> input_junctions;       //! The network's input junctions, will set them every refresh
		std::set<Output*> output_junctions;     //! The network's output junctions, will determine new state from their values during refresh. 
