...
/**
 * @brief Junctions are what connect wires, provide circuit enties with their
 * input value(s) and hold the entities output value(s).
 *
 * Junction is an abstract struct, which is implemented as either an Input or
 * Output junction.
 */
struct Junction {
public:
  ...
  /**
   * @brief Forbids constructing a new Junction object without specifying it's
   * type
   *
   */
  Junction() = delete;
   ...
   * If an output junction, then it is set by whoever owns the output (like a
   * gate).
   *
   * @note it is up to the Junction instance owner to protect access to the
   * junctin state.
   */
  Logic_Value state;
...
protected:
   ...
   */
  Junction(Type type, uint32_t id)
      : type(type), state(Logic_Value::HI_Z), index_id(id) {}
...
public:
   ...
   *
   * We cannot delete the default constructor or we lose the ability to put it
   * in a vector.
   */
  Output() : Output(0){};
