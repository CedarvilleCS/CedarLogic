 ...
 * 
 * @note Circuit may never expose a pointer. A uint32_t id number is fine but it's a weak reference.
 * @note Circuit may never throw an exception.
 * @note Circuit may not be made to respond in real-time to a function call. Rather, everything is message queue based.
 * @note Circuit is entirely responsible for it's own state and memory management.
 */
class Circuit {
public:
...
private:
	 ...
	 * 
	 * @note at the circuit level they aren't distinguished between Input and Output.
	 */
	 ...
	 * 
	 * @note They are encapsulated in unique_ptrs so it can be obvious when ownership
	 * is transferred to Network's static functions which will new and delete 
	 * Network instances.
	 * 
	 * @note They are held in a set rather than vector because they are created and 
	 * deleted more frequently than any other object and so a vector would be expected
	 * to result in much more wasted space.
	 */
