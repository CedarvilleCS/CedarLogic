...
bool cmdCreateGate::Do() {
	...
	if (logicType.size() > 0) {
		ostringstream oss;
		gCircuit->sendMessageToCore(Message(MT_CREATE_GATE, new Message_CREATE_GATE(logicType, gid)));
		...
		for (unsigned int i = 0; i < libGate.hotspots.size(); i++) {
			...
			if (libGate.hotspots[i].isInverted) {
				if (libGate.hotspots[i].isInput) {
					gCircuit->sendMessageToCore(Message(MT_SET_GATE_INPUT_PARAM, new Message_SET_GATE_INPUT_PARAM(gid, libGate.hotspots[i].name, "INVERTED", "TRUE")));
				}
				else {
					gCircuit->sendMessageToCore(Message(MT_SET_GATE_OUTPUT_PARAM, new Message_SET_GATE_OUTPUT_PARAM(gid, libGate.hotspots[i].name, "INVERTED", "TRUE")));
				}
			}
			...
			if (libGate.hotspots[i].logicEInput != "") {
				if (libGate.hotspots[i].isInput) {
					gCircuit->sendMessageToCore(Message(MT_SET_GATE_INPUT_PARAM, new Message_SET_GATE_INPUT_PARAM(gid, libGate.hotspots[i].name, "E_INPUT", libGate.hotspots[i].logicEInput)));
				}
				else {
					gCircuit->sendMessageToCore(Message(MT_SET_GATE_OUTPUT_PARAM, new Message_SET_GATE_OUTPUT_PARAM(gid, libGate.hotspots[i].name, "E_INPUT", libGate.hotspots[i].logicEInput)));
				}
			}
...
bool cmdCreateGate::Undo() {
	...
	string logicType = wxGetApp().libParser.getGateLogicType(gateType);
	if (logicType.size() > 0) {
		gCircuit->sendMessageToCore(Message(MT_DELETE_GATE, new Message_DELETE_GATE(gid)));
	}
	return true;
}
