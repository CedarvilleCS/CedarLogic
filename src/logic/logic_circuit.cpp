...
void Circuit::stepOnlyGates() {
	...
	auto updateGate = gateUpdateList.begin();
...
void Circuit::step(ID_SET< IDType > *changedWires)
{
	...
	auto gateToPoll = polledGates.begin();
	...
	auto chgWireIterator = changedWires->begin();
	...
	auto changedGatesIterator = changedGates.begin();
...
IDType Circuit::newGate(const string &type, IDType gateID) {
	...
	if (gateList.find(thisGateID) == gateList.end()) {
		...
		if (type == "AND") {
			...
			gateList[thisGateID] = GATE_PTR(new Gate_BLACK_BOX(this));
		}
		else if (type == "CLOCK") {
...
set< WIRE_PTR > Circuit::getJunctionGroup(IDType wireID) {
	...
	auto wireIDs = wireGroupIDs.begin();
